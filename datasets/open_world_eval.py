...
class OWEvaluator:
    ...
    def summarize(self, fmt='{:.06f}'):
        ...
        self.metrics["detection mAP50: "] = fmt.format(mAP50)
        self.metrics["detection mAp: "] =  fmt.format(mAP)

        print('---AP50---')
        wi = self.compute_WI_at_many_recall_level(self.all_recs, self.tp_plus_fp_cs, self.fp_os)
        print('Wilderness Impact: ' + str(wi))

        self.metrics["WI: "] = str(wi)
        self.metrics["WI"] = str(wi) # tensorboard
        ...
        self.metrics["OSE:"] = str(total_num_unk_det_as_known)
        self.metrics["OSE"] = str(total_num_unk_det_as_known) # tensorboard
        self.metrics["total_num_unk: "] = str(total_num_unk)
        ...
        self.metrics["AP50: "] = str(['%.1f' % x for x in self.AP[:, o50]])
        self.metrics["Precisions50:"] = str(['%.1f' % x for x in self.precs[50]])
        self.metrics["Recall50: "] = str(['%.1f' % x for x in self.recs[50]])
        ...
        if self.prev_intro_cls > 0:
            ...
            self.metrics["Prev class AP50: "] = str(self.AP[:, o50][:self.prev_intro_cls].mean())
            self.metrics["Prev_AP"] = str(self.AP[:, o50][:self.prev_intro_cls].mean())  # tensorboard
            self.metrics["Prev class Precisions50: "] = str(np.mean(self.precs[50][:self.prev_intro_cls]))
            self.metrics["Prev class Recall50: "] = str(np.mean(self.recs[50][:self.prev_intro_cls]))
        ...
        self.metrics["Current class AP50: "] = str(self.AP[:, o50][self.prev_intro_cls:self.prev_intro_cls + self.curr_intro_cls].mean())
        self.metrics["Current class Precisions50: "] = str(np.mean(self.precs[50][self.prev_intro_cls:self.prev_intro_cls + self.curr_intro_cls]))
        self.metrics["Current class Recall50: "] = str(np.mean(self.recs[50][self.prev_intro_cls:self.prev_intro_cls + self.curr_intro_cls]))

        self.metrics["Known AP50: "] = str(self.AP[:, o50][:self.prev_intro_cls + self.curr_intro_cls].mean())
        self.metrics["Known Precisions50: "] = str(np.mean(self.precs[50][:self.prev_intro_cls + self.curr_intro_cls]))
        self.metrics["Known Recall50: "] = str(np.mean(self.recs[50][:self.prev_intro_cls + self.curr_intro_cls]))

        self.metrics["Unknown AP50: "] = str(self.AP[:, o50][-1])
        self.metrics["Unknown Precisions50: "] =  str(self.precs[50][-1])
        self.metrics["Unknown Recall50: "] = str(self.recs[50][-1])


        for class_name, ap in zip(self.voc_gt.CLASS_NAMES, self.AP[:, o50].cpu().tolist()):
            print(class_name, fmt.format(ap))
            self.metrics[str(class_name)] = fmt.format(ap)
