...
class DeformableDETR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        for lvl in range(class_hs.shape[0]):
            ...
            if self.novelty_cls:
                outputs_class_nc = self.nc_class_embed[lvl](class_hs[lvl])
...
class SetCriterion(nn.Module):
    ...
    def loss_NC_labels(self, outputs, targets, indices, num_boxes, current_epoch, Iter, owod_targets, owod_indices, log=True):
        ...
        return losses
    def loss_labels(self, outputs, targets, indices, num_boxes, current_epoch, Iter, owod_targets, owod_indices, log=True):
    ...
    @torch.no_grad()
    def loss_cardinality(self, outputs, targets, indices, num_boxes, current_epoch, Iter, owod_targets, owod_indices):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, current_epoch, Iter, owod_targets, owod_indices):
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes, current_epoch, Iter, owod_targets, owod_indices):
    ...
    def get_loss(self, loss, outputs, targets, indices, num_boxes, epoch, Iter, owod_targets, owod_indices, **kwargs):
        loss_map = {
        ...
        return loss_map[loss](outputs, targets, indices, num_boxes, epoch, Iter, owod_targets, owod_indices, **kwargs)
    ...
    def forward(self, samples, outputs, targets, epoch, Iter, last_loss):
        ...
        if self.unmatched_boxes and epoch >= loss_epoch:
            ...
            for i in range(len(indices)):
                ...
                if "selective_region" in targets[i]:
                    selective_region = box_ops.box_cxcywh_to_xyxy(deepcopy(targets[i]['selective_region']))
                    selective_region = selective_region * torch.tensor([img_w, img_h, img_w, img_h],dtype=torch.float32).to(owod_device)
                else:
                    selective_region = torch.tensor([])
                ...
                for j, _ in enumerate(means_bb):
                    if j in unmatched_indices:
                        ...
                        xmax = bb[j, :][2].long()
                        ymax = bb[j, :][3].long()
                        information = img_feat[ymin:ymax, xmin:xmax]
                        flatten_information = torch.flatten(information)
                        means_bb[j] = torch.mean(img_feat[ymin:ymax, xmin:xmax])
                        if torch.isnan(means_bb[j]):
                            means_bb[j] = -10e10
                    else:
                         means_bb[j] = -10e10
                means_bb[means_bb < 0] = 0
                means_bb = torch.div(means_bb, torch.max(means_bb))
        ...
        for loss in self.losses:
            kwargs = {}
            losses.update(self.get_loss(loss, outputs, targets, indices, num_boxes, epoch, Iter, owod_targets, owod_indices, **kwargs))
            ...
            for p, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                if self.unmatched_boxes and epoch >= loss_epoch:
                    ...
                    for i in range(len(indices)):
                        ...
                        if "selective_region" in targets[i]:
                            selective_region = box_ops.box_cxcywh_to_xyxy(deepcopy(targets[i]['selective_region']))
                            selective_region = selective_region * torch.tensor([img_w, img_h, img_w, img_h],
                ...
                for loss in self.losses:
                    ...
                    if loss == 'labels':
                        ...
                        kwargs['log'] = False
                    l_dict = self.get_loss(loss, aux_outputs, targets, indices, num_boxes, epoch, Iter, owod_targets, owod_indices, **kwargs)
